...
class SuperagentTransactionController extends Controller
{
    ...
    public function index(Request $request, $id)
    {
        ...
        $superagentTransactionsQuery = superagentTransaction::with(['superagent']);

        if ($request->filled('from')) {
            $superagentTransactionsQuery->where('created_at', '>=', $request->from);
        }

        if ($request->filled('to')) {
            $superagentTransactionsQuery->where('created_at', '<=', $request->to);
        }

        $superagentTransactions = $superagentTransactionsQuery->where('superagent_id', $id)->latest()->get();
