...
<table class="table table-striped" id="extensions_id" style="width:100%">
    ...
    <tbody id="serviceTableRows">
        <?php $__empty_1 = true; $__currentLoopData = isset($booking_services) ? $booking_services : []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $service): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <tr id="service_<?php echo e($service->id); ?>">
                ...
                <td>
                    ...
                    <?php if(($service->payment_type ?? null) === 'supplier'): ?>
                        <br><small class="text-info">مورد<?php echo e($service->supplier?->name ? ': ' . $service->supplier->name : ''); ?></small>
                    <?php endif; ?>
                </td>
                <td>
        ...
        <?php if(isset($expensesServices)): ?>
            <?php $__currentLoopData = $expensesServices; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $expense): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <tr id="agent_expense_<?php echo e($expense->id); ?>">
                    ...
                    <td>
                        <?php echo e($expense->service?->name ?? __('main.not_found')); ?>

                    </td>
                    <td>
                        <?php echo e(filled($expense->notes) ? $expense->notes : '—'); ?>
        ...
        <?php if(isset($supplierReceipts)): ?>
            <?php $__currentLoopData = $supplierReceipts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $receipt): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <tr id="supplier_receipt_<?php echo e($receipt->id); ?>">
                    <td>
                        <?php echo e($receipt->id); ?>

                    </td>
                    <td>
                        إيصال مورد<?php echo e($receipt->supplier?->name ? ': ' . $receipt->supplier->name : ''); ?>

                        <?php if($receipt->supplier_invoice_number): ?>
                            <br><small class="text-muted">فاتورة: <?php echo e($receipt->supplier_invoice_number); ?></small>
                        <?php endif; ?>
                    </td>
                    <td>
                    ...
                    <td class="services_total_price" data-price="<?php echo e($receipt->cost); ?>">
                        <?php echo e($receipt->cost); ?>
                    ...
                    <td>
                        <div class="d-flex gap-2">
                            <?php if(auth()->user()->hasPermissionTo('suppliers.udpate') || auth()->user()->hasPermissionTo('suppliers.update') || auth()->user()->roles->pluck('name')->contains('Admin')): ?>
                                <a href="<?php echo e(route('receipts.edit', $receipt)); ?>"
                                   class="btn btn-icon btn-light btn-hover-primary btn-sm">
                                    <i class="fas fa-edit text-primary"></i>
                                </a>
                            <?php endif; ?>
